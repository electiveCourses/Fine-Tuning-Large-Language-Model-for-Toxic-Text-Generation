...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate LoRA-finetuned Qwen on prompts with toxicity scoring.")
    parser.add_argument('--lora-model-path', type=str, default='results/checkpoint-1000', help='Path to LoRA-finetuned model checkpoint')
    ...
    parser.add_argument('--output', type=str, default='data/processed/qwen_lora_eval_with_toxicity.csv', help='Output CSV file')
...
def main():
    ...
    # Load LoRA-finetuned Qwen model
    model = AutoModelForCausalLM.from_pretrained(args.lora_model_path, trust_remote_code=True)
    tokenizer = AutoTokenizer.from_pretrained(args.lora_model_path, trust_remote_code=True)
